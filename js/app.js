...
<div style="background:#fff8f8;border:1px solid rgba(210,0,21,0.2);border-left:4px solid #d20015;border-radius:4px;padding:12px 16px;margin-bottom:14px;">
  <div style="display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:10px;">
    <div style="flex:1;min-width:0;">
      ...
      <div style="font-size:0.65rem;font-weight:800;color:#334155;text-transform:uppercase;letter-spacing:1px;margin-bottom:5px;"><i class="fas fa-arrows-left-right me-1"></i>타 부문 연계 지적 — ${crSection} 부문 영향 (${crossSectionFindings.length}건)</div>
      <div style="display:flex;gap:5px;flex-wrap:wrap;">
        ${crossSectionFindings.map(f=>`<span onclick="openFindingDetailFromCR('${f.isarp}')" style="background:#f1f5f9;color:#334155;border:1px solid rgba(51,65,85,0.3);padding:3px 10px;border-radius:4px;font-size:0.68rem;font-weight:800;cursor:pointer;display:inline-flex;align-items:center;gap:4px;" title="${f.desc||''} (원 부문: ${f.sect})"><i class="fas fa-link" style="font-size:0.55rem;"></i>${f.isarp}<span style="background:rgba(51,65,85,0.1);color:#334155;font-size:0.52rem;padding:0 4px;border-radius:2px;margin-left:3px;">${f.sect}</span></span>`).join('')}
      </div>` : ''}
    </div>
...
<div id="cr-ev-${_esc(e.id)}" class="cr-ev-panel" style="display:none;">
  ...
  <textarea class="cr-ev-note" placeholder="추가 메모 (준비 현황, 파일명 등)..."
    ...
    const rowBorder = isRepeated ? 'border-left-color:#d20015;border-left-width:4px;' : hasFinding ? 'border-left-color:#d20015;border-left-width:3px;' : `border-left-color:${st.color};`;
...
function renderISMAnalysis() {
  ...
  const TECH_CHANGES = [
    { sect:'ORG', items:[
      ...
      { code:'ORG 1.7.2',       tag:'기술변경', desc:'ERP 관리자는 ERP 개발·구현·유지에 필요한 훈련 및 권한 보유 의무화; AA2: 직무·책임·권한·훈련 기록 집중 감사' },
      { code:'ORG 1.7.3',       tag:'기술변경', desc:'ERP 이벤트 시 협력할 파트너 기관 식별 및 역할 정의 요건 신설; AA1·AA2: ERP 매뉴얼 파트너 목록 및 업무 할당 감사' },
      { code:'ORG 1.7.4',       tag:'기술변경', desc:'운항 공항별 로컬 ERP 필수화 — 인도주의 서비스, ERP 이벤트 정보 처리, 스테이션 ERP 리더 지명, 훈련 요건 포함' },
      { code:'ORG 1.7.5',       tag:'기술변경', desc:'ERP에 탑승객 관리(souls on board)·매니페스트·데이터 프라이버시 법령 준수 프로세스 의무화' },
      { code:'ORG 1.7.6',       tag:'기술변경', desc:'인도주의 지원 및 가족 지원(Family Assistance) 착수 프로세스 — 발동·해제, 긴급지원센터 운영, 유류품 관리, 유해 송환, 심리지원 포함' },
      { code:'ORG 1.7.7',       tag:'기술변경', desc:'ERP 대응 직원 심리지원(장기 상담 포함) 권고 — 사건 미참여 직원도 적용 가능하도록 명확화' },
      { code:'ORG 2.2.3',       tag:'기술변경', desc:'코드쉐어·습식임대(Wet Lease) 운영 시 조건부 적용으로 수정 — IOSA 등록 제3사 운항사로부터 습식임대 시 컴플라이언스 고려사항 설명 추가' },
    ...
    { sect:'FLT', items:[
      ...
      { code:'FLT 3.11.17',     tag:'기술변경', desc:'"AGL" 용어 삭제 — 정확성 향상을 위한 수정' },
      { code:'FLT 3.11.20',     tag:'내용추가',  desc:'FMS 입력을 위한 독립적 성능 계산(independent performance calculations) 또는 입력값에 관한 신규 sub-item 추가; Guidance도 동일하게 개정' },
      { code:'FLT 3.11.32(iii)',tag:'내용추가',  desc:'고도계 오류 유형에 ATC 전송 오류(ATC transmission errors) 명시적 포함; Guidance에 완화 방안 및 참고자료 추가' },
      { code:'FLT 3.11.59(iv)', tag:'기술변경', desc:'안정화 접근 기준 사전 계획·브리핑 — sub-item(iv) 조건부 적용으로 변경하여 운항사별 해당 여부 판단 가능; Guidance에 조건부 예시 추가' },
      { code:'FLT 3.12.6',      tag:'기술변경', desc:'(RP→ST 격상) 런웨이 이탈(Runway Excursion) 방지 지침 의무화; Guidance에 최신 참조 문서 업데이트' },
    ...
    { sect:'DSP', items:[
      ...
      { code:'DSP 4.5.1',       tag:'기술변경', desc:'DSP 4.5.2 내용 흡수 통합 — 3개 sub-item 신설로 ETOPS/EDTO 관리 요건 효율적 정렬; Guidance도 함께 업데이트' },
    ...
    { sect:'MNT', items:[
      ...
      { code:'MNT Table 4.11(xiv)',tag:'기술변경',desc:'25시간 CVR Active 구현 기한 Note 삭제 — 2026년 만료로 더 이상 유효하지 않음' },
      { code:'MNT Table 4.11(xv)', tag:'기술변경',desc:'DLC Active 구현 기한 Note 삭제 — 2026년 만료; 적용성 명확화 유지' },
    ...
    { sect:'GRH', items:[
      ...
      { code:'GRH 3.6.6',       tag:'기술변경', desc:'잘못된 상호 참조(incorrect references) 삭제' },
      { code:'GRH 3.7.12',      tag:'신규요건',  desc:'(SEC 3.1.3에서 이관) 운항 보안 요건 — GRH 섹션으로 통합 관리' },
      { code:'GRH 3.7.13',      tag:'신규요건',  desc:'(SEC 3.3.3에서 이관) 운항 보안 요건 — GRH 섹션으로 통합 관리' },
    ...
    { sect:'CGO', items:[
      ...
      { code:'CGO 3.6.1 GM',    tag:'기술변경', desc:'"special cargo"와 "special loads" 용어 구분 및 사용 기준 명확화' },
    ...
    { sect:'SEC', items:[
      ...
      { code:'SEC 1.10.1A',      tag:'신규요건',  desc:'(신규 Standard) 소제목 "Quality Control" — SeMS 품질 통제 프로그램 의무화; 신규 AA 및 GM 함께 신설' },
      { code:'SEC 1.11.3(iii)(v)',tag:'기술변경',desc:'보안 교육 프로그램 — (iii) 위험 평가 및 (v) 품질 관리 항목 추가' },
      { code:'SEC 3.1.1',        tag:'기술변경', desc:'배타적 제한 구역 통제 명확화 — 신규 sub-item(ii) 추가: 인원 검색(screening of personnel) 요건' },
      { code:'SEC 3.4.2',        tag:'기술변경', desc:'기존 SEC 3.4.5 조항 번호 재배치 — 내용 변경 없이 3.4.2로 이동; 기존 3.4.5 자리는 placeholder 처리' },
      { code:'SEC 3.4.3(iii)',   tag:'내용추가',  desc:'신규 sub-item(iii) — 통과(transit) 승객 및 기내 수하물 무단 간섭으로부터의 보호 요건' },
      { code:'SEC 3.4.6',        tag:'기술변경', desc:'보안 인원의 행동 인식(behavioral awareness) 관련 문구 개정; AA1·AA3 및 GM도 일관성 있게 수정' },
      { code:'SEC 3.6.1',        tag:'내용추가',  desc:'환승 수하물(transfer hold baggage) 보안 검색 요건 추가 — 신규 sub-items(iii)(iv)(v) 신설; AA1·AA3 및 GM 함께 개정' },
      { code:'SEC 3.6.2',        tag:'기술변경', desc:'환승 수하물 검색(transfer hold baggage screening) 내용 포함으로 개정' },
      { code:'SEC 3.7.1',        tag:'기술변경', desc:'우편물 보안 통제(mail security controls) 포함; AA1·AA2·AA3 모두 일관성 있게 개정' },
      { code:'SEC 3.8.1',        tag:'기술변경', desc:'모든 해당 운항사에 적용 가능하도록 적용 범위 확대' },
    ]},
  ];
...
<!-- ④ 기술 변경 — 섹션 패널 + 유형별 아이콘 미니카드 -->
<div id="ism-acc-tech" style="display:none;margin-bottom:12px;">
  <div class="w-card" style="border-left:4px solid #6b7280;overflow:hidden;">
    <div class="w-card-header" style="flex-wrap:wrap;gap:8px;">
      ...
      <div style="display:flex;gap:5px;flex-wrap:wrap;">
        <span style="padding:2px 9px;border-radius:8px;font-size:0.6rem;font-weight:700;background:#f1f5f9;color:#475569;border:1px solid #cbd5e1;"><i class="fas fa-wrench me-1"></i>기술변경</span>
        <span style="padding:2px 9px;border-radius:8px;font-size:0.6rem;font-weight:700;background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe;"><i class="fas fa-plus me-1"></i>내용추가</span>
        <span style="padding:2px 9px;border-radius:8px;font-size:0.6rem;font-weight:700;background:#fff1f2;color:#d20015;border:1px solid #fca5a5;"><i class="fas fa-star me-1"></i>신규요건</span>
      </div>
    </div>
    <div class="w-card-body" style="padding:0;">
      ${TECH_CHANGES.map(g=>{const sc=SECT_COLOR[g.sect]||'#374151';const si=SECT_ICON[g.sect]||'fa-circle';const nNew=g.items.filter(i=>i.tag==='신규요건').length;const nAdd=g.items.filter(i=>i.tag==='내용추가').length;const TAG_ICON={'기술변경':'fa-wrench','내용추가':'fa-plus','신규요건':'fa-star','삭제':'fa-ban'};return `
      <div style="border-bottom:1px solid #f1f5f9;">
        <div style="padding:11px 20px;display:flex;align-items:center;gap:12px;cursor:pointer;background:#fafafa;" onmouseenter="this.style.background='#f3f4f6'" onmouseleave="this.style.background='#fafafa'" onclick="(function(el){var b=el.nextElementSibling;var op=b.style.display==='block';b.style.display=op?'none':'block';el.querySelector('.sc-chev').style.transform=op?'rotate(0deg)':'rotate(180deg)';})(this)">
          <div style="width:38px;height:38px;background:${sc};border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-shadow:0 2px 8px ${sc}55;">
            <i class="fas ${si}" style="color:white;font-size:0.78rem;"></i>
          </div>
          <div style="flex:1;">
            <div style="font-size:0.85rem;font-weight:700;color:#1e293b;">${SECT[g.sect]||g.sect}</div>
            <div style="font-size:0.62rem;color:#94a3b8;margin-top:1px;display:flex;align-items:center;gap:6px;">
              <span style="font-family:monospace;color:${sc};font-weight:700;">${g.sect}</span>
              <span>·</span><span>총 ${g.items.length}건 변경</span>
            </div>
          </div>
          <div style="display:flex;align-items:center;gap:5px;flex-shrink:0;">
            ${nNew>0?`<span style="background:#fff1f2;color:#d20015;border:1px solid #fca5a5;padding:2px 9px;border-radius:10px;font-size:0.6rem;font-weight:700;"><i class="fas fa-star me-1" style="font-size:0.5rem;"></i>${nNew}건</span>`:''}
            ${nAdd>0?`<span style="background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe;padding:2px 9px;border-radius:10px;font-size:0.6rem;font-weight:700;"><i class="fas fa-plus me-1" style="font-size:0.5rem;"></i>${nAdd}건</span>`:''}
            <i class="fas fa-chevron-down sc-chev" style="color:#d1d5db;font-size:0.62rem;transition:transform .2s;margin-left:3px;"></i>
          </div>
        </div>
        <div style="display:none;padding:14px 16px 18px;background:#fdfdfe;">
          <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:10px;">
            ${g.items.map(item=>{const tb=TAG_BORDER[item.tag]||'#e2e8f0';const ts=TAG_STYLE[item.tag]||'';const ti=TAG_ICON[item.tag]||'fa-circle';return `
            <div style="border-radius:10px;background:#fff;border:1px solid #eaecf0;border-left:4px solid ${tb};padding:12px 14px;box-shadow:0 1px 4px rgba(0,0,0,.04);">
              <div style="display:flex;align-items:center;gap:7px;margin-bottom:8px;">
                <div style="width:24px;height:24px;border-radius:6px;display:flex;align-items:center;justify-content:center;flex-shrink:0;${ts}border:none!important;">
                  <i class="fas ${ti}" style="font-size:0.6rem;"></i>
                </div>
                <span style="font-family:monospace;font-size:0.82rem;font-weight:900;color:#1e293b;flex:1;">${item.code}</span>
                <span style="padding:2px 7px;border-radius:8px;font-size:0.58rem;font-weight:700;${ts}">${item.tag}</span>
              </div>
              <div style="font-size:0.76rem;color:#4b5563;line-height:1.65;padding-left:31px;">${item.desc}</div>
            </div>`;}).join('')}
          </div>
        </div>
      </div>`;}).join('')}
